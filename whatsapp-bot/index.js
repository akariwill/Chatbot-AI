...
function getGreetingResponse(text, forceSalutation = false) {
    ...
    if (greetings.includes(normalizedText)) {
        // This part is for when the user's message IS a greeting
        return `${salutation} juga! 😊 Ada yang bisa aku bantu? Kamu bisa tanya misalnya:\n- Paket WiFi yang tersedia\n- Cara pembayaran\n- Hubungi teknisi\n\nSilakan tanyakan ya~`;
    }

    if (forceSalutation) {
        // This part is just to get the salutation for other messages
        return salutation;
    }
...
async function startSock() {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // Save user message to history. From this point on, the user is no longer "new".
        ...
        // Don't send a greeting reply if the user was new, we already sent a welcome.
